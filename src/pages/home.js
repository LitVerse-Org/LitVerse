  ...
  return (
    <Layout>
      <div style={{ textAlign: 'center' }}>
        <button onClick={toggleSessionInfo} style={{ backgroundColor: '#007bff', color: 'white', padding: '5px 10px', borderRadius: '5px', margin: '10px' }}>
          Show Session Info
        </button>
        {showSessionInfo && <div ref={sessionInfoRef} style={{ backgroundColor: '#f1f1f1', padding: '10px', borderRadius: '5px' }}>
          ...
          <div key={index} style={{ border: '1px solid #ccc', borderRadius: '5px', padding: '10px', margin: '10px' }}>
            ...
            <ul style={{ color: 'white' }}>
              {user.posts.map((post, postIndex) => (
                <li key={postIndex} style={{ marginBottom: '10px', padding: '5px', borderRadius: '3px', backgroundColor: '#333' }}>
                  {post.content}
                </li>
              ))}
            </ul>
